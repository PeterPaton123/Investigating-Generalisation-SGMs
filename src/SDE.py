...
class SDE():
    ...
    def step_milstein(self):
        ...
        brownian_motion_squared_samples = jnp.multiply(brownian_motion_squared_samples, brownian_motion_squared_samples)
        samples_at_t_plus_dt = prevXs + self.dt * jax.vmap(uPartial)(prevXs) + jnp.multiply(jax.vmap(sPartial)(prevXs), brownian_motion_samples) + 0.5 * jnp.multiply(jnp.multiply(jax.vmap(sPartial)(prevXs), jax.vmap(jax.grad(sPartial))(prevXs)), (brownian_motion_squared_samples - self.dt))
        self.samples = np.column_stack((self.samples, samples_at_t_plus_dt))
        self.ts = np.append(self.ts, t)
        self.T += self.dt
