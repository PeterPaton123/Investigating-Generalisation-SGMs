...
"""
def sample_ellipse(num_samples, x0, y0, rngkey, offset=False, A=jnp.eye(2)):
    """Samples from the unit ellipse centered at x0 y0, angles split equally around point.
...
def main():
    ...
    samples = sample_ellipse(num_samples, 0, 0, sample_rng, offset=False, A=A)
    samples_2 = sample_ellipse(num_samples, 0, 0, sample_rng, offset=True, A=A)
    ...
    true = sample_ellipse(100, 0, 0, sample_rng, offset=False, A=A)
    ax.scatter(
        true[:, index[0]], true[:, index[1]],
        color='blue', label="True", marker='.', linewidths=1.0, alpha=0.5
    )
    ...
    # Train with score matching
        ...
        q_samples_projected_back = inverse_transformation(q_samples, A)

        plot_heatmap(samples=q_samples_projected_back[:, [0, 1]], area_min=-2, area_max=2, fname=f"bin/heatmap trained score-{1000*(i+1)}-projected-back")
