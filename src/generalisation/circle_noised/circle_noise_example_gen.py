...
"""
def sample_circle(num_samples, x0, y0, rngkey, noise_sd, offset=False):
    ...
    noise = random.normal(rngkey, shape=(num_samples,)) * noise_sd
    samples = vmap(lambda in_: in_[0] * (1 + in_[1]))((samples, noise))
    return samples
...
def main():
    ...
    num_samples = 8
    samples = sample_circle(num_samples, 0, 0, sample_rng, 0.075, offset=False)
    samples_2 = sample_circle(num_samples, 0, 0, sample_rng, 0.075, offset=True)
    ...
    generalisation_metric_simple = np.zeros(10)
    generalisation_metric = np.zeros(10)
    generalisation_metric_true = np.zeros(10)
    for i in range(10):
    # Train with score matching
        ...
        plot_score(score=trained_score, t=0.01, area_min=-2, area_max=2, fname=f"bin/trained score epoch-{1500*(i+1)}")
        ...
        plot_heatmap(samples=q_samples[:, [0, 1]], area_min=-2, area_max=2, fname=f"bin/heatmap trained score-{1500*(i+1)}")
