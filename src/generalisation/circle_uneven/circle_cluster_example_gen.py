...
def sample_circle_uneven(num_samples, x0, y0, rngkey, offset=False):
    """Samples from the unit circle centered at x0 y0, angles split equally around point.
    Offset will rotate the samples and generate different samples along the same manifold
    ...
    Returns a (N_samples, 2) array of samples
    """
    alphas = jnp.linspace(0.03, 0.97, num_samples)
    normal = Partial(jax.scipy.stats.norm.ppf, loc=jnp.pi/4, scale=jnp.sqrt(jnp.pi * 0.75))
    alphas = vmap(normal)(alphas)
    xs = x0 + jnp.cos(alphas)
    ys = y0 + jnp.sin(alphas)
    samples = jnp.stack([xs, ys], axis=1)
    return samples
...
def main():
    ...
    num_samples = 40
    samples = sample_circle_uneven(num_samples, 0, 0, sample_rng, offset=False)
    ...
    true = sample_circle(50, 0, 0, sample_rng, offset=False)
    ax.scatter(
    ...
    generalisation_metric_simple = np.zeros(CONST, dtype=object)
    generalisation_metric = np.zeros(CONST, dtype=object)
    generalisation_metric_true = np.zeros(CONST, dtype=object)
