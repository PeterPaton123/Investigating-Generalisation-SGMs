...
xs_1 = np.linspace(-8, 12, num=200)
p_x0s = 0.3 * pdf1(xs_1) + 0.7 * pdf2(xs_1)  
...
def plot(ts, samples_at_t, xs, expected_pdf_at_time_t, num_samples, timesteps_per_second, time_range):
    ...
    axes[0].plot(xs_1, p_x0s, color = 'r', linestyle='--')
    ...
    ## Expected pdf:
    mesh = axes[2].pcolormesh(ts[1:], xs, expected_pdf_at_time_t.T, cmap=cmap, norm=LogNorm(vmax=1.5))
